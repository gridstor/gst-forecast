...
import { houstonCalibrationData, calibrationChartOptions } from '../utils/charts/calibrationComparison'
...
const handleDownload = () => {
  const csvContent = 'data:text/csv;charset=utf-8,' + 
    chartData.value.labels.map((label, idx) => {
      ...
      return `${label},${rangeData.y[0]},${rangeData.y[1]},${rangeData.p50},${p50Data},${actualData}`;
    }).join('\n')
  
  const encodedUri = encodeURI(csvContent)
  const link = document.createElement('a')
  link.setAttribute('href', encodedUri)
  link.setAttribute('download', 'hidden_lakes_calibration.csv')
