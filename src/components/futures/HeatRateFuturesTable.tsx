...
const HeatRateFuturesTable: React.FC<Props> = ({ contractTerm }) => {
  ...
  const fetchData = async (peakHour: string) => {
    try {
      setLoading(true);
      const response = await fetch(`/api/futures/heat-rate?peakHour=${peakHour}&contractTerm=${contractTerm}`);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-semibold text-[#2A2A2A]">Heat Rate Futures</h3>
      ...
      <div className="text-right mt-2 space-y-1">
        <div className="text-xs text-gray-400">
          Source: {data.metadata?.powerSource || 'OTCGH'} {data.metadata?.latestCurveDate || 'N/A'}
        </div>
        <div className="text-xs text-gray-400">
