...
const PowerFuturesTable: React.FC<Props> = ({ contractTerm }) => {
  ...
  const fetchData = async (peakHour: string) => {
    try {
      setLoading(true);
      const response = await fetch(`/api/futures/power?peakHour=${peakHour}&contractTerm=${contractTerm}`);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-semibold text-[#2A2A2A]">Power Futures</h3>
          <p className="text-sm text-[#6B7280]">
            Mid forward curve prices in {data.metadata?.units || '$/MWh'}
          </p>
        </div>
        ...
        <table className="w-full text-sm border border-gray-200 rounded-lg">
          ...
          <tbody className="font-mono">
            {data.data?.tableData.map((row, index) => (
              ...
              >
                <td className="p-3 font-semibold text-[#2A2A2A] sticky left-0 bg-white hover:bg-gray-50">
                  {row.market}
                </td>
                {data.data?.years.map(year => (
                  <td 
      ...
      {/* Source text at bottom right */}
      <div className="text-right mt-2 space-y-1">
        <div className="text-xs text-gray-400">
          Source: {data.metadata?.dataSource || 'OTCGH'} {data.metadata?.latestCurveDate || 'N/A'}
        </div>
        <div className="text-xs text-gray-400">
