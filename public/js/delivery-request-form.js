// Delivery Request Form JavaScript
...
class DeliveryRequestForm {
  ...
  initializeElements() {
    // Form elements
    this.form = document.getElementById('deliveryRequestForm');
  ...
  setDefaultDates() {
    ...
    const now = new Date();
    const dueDateDefault = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000);
    this.dueDate.value = this.formatDate(dueDateDefault);
  ...
  validateDueDate() {
    ...
    if (this.dueDate.value) {
      if (dueDate < today) {
        this.dueDate.setCustomValidity('Due date cannot be in the past');
        return false;
      } else {
  ...
  validateForm() {
    ...
    if (!data.responsibleTeam) errors.push('Responsible Team is required');
    if (!data.deliveryPeriodStart) errors.push('Delivery Period Start is required');
    if (!data.deliveryPeriodEnd) errors.push('Delivery Period End is required');
  ...
  async handlePreview() {
    ...
    try {
      ...
      const response = await fetch('/api/delivery-request/preview', {
        method: 'POST',
  ...
  displayPreview(previewData) {
    const { curveDefinition, deliveryRequest, deliverySpec, validation } = previewData;
    
    this.previewContent.innerHTML = `
      <!-- Validation Status -->
      <div class="mb-6 p-4 ${validation.allValid ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'} border rounded-md">
        <h4 class="font-medium ${validation.allValid ? 'text-green-900' : 'text-red-900'} mb-2">Validation Status</h4>
        <div class="text-sm space-y-1">
          <div class="flex items-center">
            <span class="w-4 h-4 mr-2">${validation.dueDateValid ? '✅' : '❌'}</span>
            <span>Due date: ${validation.dueDateValid ? 'Valid' : 'Invalid'}</span>
          </div>
          <div class="flex items-center">
      ...
      <div class="border border-gray-200 rounded-lg p-4 mb-4">
        <h4 class="font-medium text-gray-900 mb-3 flex items-center">
          Delivery Request
          <span class="ml-2 px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded">New Request</span>
        </h4>
        <div class="grid grid-cols-2 gap-4 text-sm">
          <div><strong>Due Date:</strong> ${new Date(deliveryRequest.dueDate).toLocaleDateString()}</div>
          <div><strong>Requested By:</strong> ${deliveryRequest.requestedBy}</div>
          <div><strong>Responsible Team:</strong> ${deliveryRequest.responsibleTeam}</div>
          <div><strong>Priority:</strong> ${deliveryRequest.priority}/5</div>
          <div><strong>Delivery Format:</strong> ${deliveryRequest.deliveryFormat}</div>
          ${deliveryRequest.deliveryNotes ? `<div class="col-span-2"><strong>Notes:</strong> ${deliveryRequest.deliveryNotes}</div>` : ''}
        </div>
      </div>

      <!-- Curve Definition -->
      <div class="border border-gray-200 rounded-lg p-4 mb-4">
        <h4 class="font-medium text-gray-900 mb-3">Curve Definition</h4>
        <div class="grid grid-cols-2 gap-4 text-sm">
          ...
          <div><strong>Type:</strong> ${curveDefinition.curveType}</div>
          <div><strong>Battery Duration:</strong> ${curveDefinition.batteryDuration}</div>
          <div><strong>Scenario:</strong> ${curveDefinition.scenario}</div>
        </div>
      </div>

      <!-- Technical Specifications -->
      <div class="border border-gray-200 rounded-lg p-4">
        <h4 class="font-medium text-gray-900 mb-3">Technical Specifications</h4>
        <div class="grid grid-cols-2 gap-4 text-sm">
          <div><strong>Delivery Start:</strong> ${new Date(deliverySpec.deliveryPeriodStart).toLocaleString()}</div>
          <div><strong>Delivery End:</strong> ${new Date(deliverySpec.deliveryPeriodEnd).toLocaleString()}</div>
          <div><strong>Duration:</strong> ${deliverySpec.deliveryDuration} days</div>
          <div><strong>Granularity:</strong> ${deliverySpec.granularity}</div>
          <div><strong>Version:</strong> ${deliverySpec.instanceVersion}</div>
          ${deliverySpec.degradationStartDate ? 
            `<div><strong>Degradation Start:</strong> ${new Date(deliverySpec.degradationStartDate).toLocaleDateString()}</div>` : 
  ...
  async handleDirectCreate() {
    ...
    await this.createDeliveryRequest();
  }
  ...
  async createDeliveryRequest() {
    ...
    try {
      this.showLoading('Creating delivery request...');
      this.closePreview();
      ...
      if (response.ok) {
        this.hideLoading();
        this.showSuccess(`Delivery request created successfully! Request ID: ${result.deliveryRequestId}`);
  ...
  handleCancel() {
    if (confirm('Are you sure you want to cancel? All form data will be lost.')) {
      window.location.href = '/delivery-management';
    }
  }
